...
function getConfigFromBabel(directory, babelrcName = 'babel.config.js') {
  const babelConfigPath = babelrcName && path.join(directory, babelrcName);

  let babelConfig =
    babelConfigPath && fs.existsSync(babelConfigPath)
      ? JSON5.parse(
          fs
            .readFileSync(babelConfigPath, 'utf8')
 ...
 * @param  {string} [babelConfigFile] - the name of the babel config file
 * @return {object}
 */
exports.resolve = (
  source,
  ...
  const isValidConfiguration =
    ...
    (isObject(config) &&
      (Object.prototype.hasOwnProperty.call(config, 'rootPathPrefix') ||
        Object.prototype.hasOwnProperty.call(config, 'rootPathSuffix')));
